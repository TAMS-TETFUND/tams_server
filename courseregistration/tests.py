...
class CourseRegistrationListTestCase(TestCase):
    ...
    def test_course_registration_api_obj_creation(self):
        ...
        request = self.factory.post(
            "course-registrations/", fixtures.course_registration_api_fixture
        )
        response = CourseRegistrationList.as_view()(request)
...
class CourseRegistrationDetailTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.course_reg = CourseRegistration.objects.create(
            **fixtures.course_registration_fixture
        )

    def test_course_registration_detail(self):
        request = self.factory.get(
            "course-registrations/{}/".format(self.course_reg.pk)
        )
        response = CourseRegistrationDetail.as_view()(
            request, pk=self.course_reg.pk
        )
